  ...
  if (args[0] === 'summoner') {
    ...
    if (data.name) {
      ...
      if (data.rank.title !== 'Unranked') {
        temp.addField('Points', data.rank.info.points)
    ...
    c = cheerio.load(await $.fetchHTML(`https://www.leaguespy.net/league-of-legends/champion/${args[1]}/stats`))
...
Searches.prototype.dota2 = async function (args) {
  ...
  var mostPlayed = []
  var record = c('.header-content-secondary>dl').eq(3).find('.game-record').text().split('-')
